...
def start(message):
    ...
    welcome_message = (
        f"Привет, {user_name}! Я бот для поиска новостей.\n\n"
        "Вы можете получить последние новости по следующим категориям:\n"
        "🪙 Крипто\n"
        "💻 Технологии\n"
        "💵 Финансы\n"
    ...
    start_button = types.InlineKeyboardButton(text='Запустить', callback_data='start_bot')
    markup.add(start_button)

    # Отправляем приветственное сообщение с кнопкой "Запустить"
...
@bot.callback_query_handler(func=lambda call: call.data in ['cryptocurrency', 'technology', 'finance', 'search_by_keyword'])
