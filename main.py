...
def suggest_currencies(input_str, currency_dict):
    prioritized = ['bitcoin', 'ethereum', 'tether', 'usd-coin', 'solana', 'cardano', 'polkadot', 'ripple', 'dogecoin',
                   'megaton-finance-wrapped-toncoin', 'mantle']
    ...
    # Фильтруем только основные криптовалюты и допустимые варианты
    filtered_matches = [k for k in exact_matches + startswith_matches + partial_matches if
                        k in prioritized or is_supported_currency(k, currency_dict)]

    sorted_matches = sorted(filtered_matches, key=lambda x: (x not in prioritized, x))
